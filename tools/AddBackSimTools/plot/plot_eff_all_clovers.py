...
from libLists import list_of_lut_2024 as list_of_lut_2024
from libColorsAnsi import *

color_index = 0
plotFold = 0 #numeration: 0 1 2 3
            ...
            for source in list_of_sources:
                if source in js_new[index]:
                    ...
                    for el in js_new[index][source]:
                        ...
                        clover_name = list_of_lut_2024['S{}'.format(color_index + 1)]
                        # print(clover_name)
                        # sys.exit()

                        label = 'Clover {} [S{}]'.format( clover_name, color_index + 1)
...
plt.title('Efficiency {} fold '.format(cloverName, pltFold))
plt.xlabel('E$\gamma$')
plt.ylabel('Efficiency, %')
plt.legend(loc='upper right', fontsize='medium', shadow=False, ncol=2)

file_name_eff = 'all_eff_fold_{}.{}'.format(pltFold, opt)
