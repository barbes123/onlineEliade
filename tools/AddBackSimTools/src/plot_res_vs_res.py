...
try:
    s = f'S{server}'
    cloverID = list_of_lut_2024[s]
    print(f'Clover Name {cloverID} server S{server}')
except:
...
#else:
#    print(f"No CloverID provided, default {cloverID}")
...
plt.plot([2, max_val+0.1], [2, max_val+0.1	], 'k--')
...
plt.tight_layout()
plt.savefig(f'../figures_src/res_vs_res_Clover{cloverID}.png', dpi=300)
plt.savefig(f'../figures_src/res_vs_res.png', dpi=300)
plt.show()
